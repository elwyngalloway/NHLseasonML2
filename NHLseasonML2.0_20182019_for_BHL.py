...
NHLseasonML 2.0 - Predicting 20182019 Multi-stat
DEVELOPMENT Version

Predict multi-stat production of NHL players for the 20182019 season.

Key parameters used for this machine learning algorithm:
    3 years of lag
    XX Layers of Neurons
    XX Neurons in each layer
    XX Epochs
    Batch size of XX
    XX Realizations simulated
    Forecast for players who have scored XX points in a season
...
with conn:
    ...
    cur.execute("SELECT playerId FROM s_skater_summary WHERE points > 60 \
...
def modelrun(modelfrom, predictfrom):
    ...
    # Inform algorithm that -999 represents non-values
    ...
    # train network
    history = model.fit(train_ind, train_resp, epochs=20, batch_size=20, validation_data=(test_ind, test_resp),verbose=0, shuffle=False)
...
for i in range(numiters):
    ...
    if i == 0:
        result = np.expand_dims(modelrun(modelarrayfrom, predictarrayfrom), axis=3)
    else:
        result = np.concatenate((result,np.expand_dims(modelrun(modelarrayfrom, predictarrayfrom), axis=3)),axis=3)

#%% FIGURE OUT WHAT TO SAVE - PROBABLY A NUMPY ARRAY OF ALL PREDICTIONS


np.save('20182019_points_goals_L03N15VE40B20.npy',result)
