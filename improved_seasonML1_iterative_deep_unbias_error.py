...
with conn:
    ...
    cur.execute("SELECT playerId FROM s_skater_summary WHERE points > 50 \
                ...
                AND seasonID NOT IN (20172018)") # I usually use points > 50
...
##%% Calculate error for mean of only first two lag predictions
#
#L12RMSEmeans =np.empty((result.shape[2]))
#
#L12meanresult = np.zeros((result.shape[0],result.shape[2]))
#
#for iteration in range(result.shape[2]):
#    for player in range(result.shape[0]):
#        L12meanresult[player,iteration] = np.mean(result[player,:-1,iteration][np.ma.masked_greater(result[player,:-1,iteration],2).mask])
#    
#    L12RMSEmeans[iteration] = np.sqrt(mean_squared_error(L12meanresult[:,iteration],actual))
#
#L12error = np.mean(L12RMSEmeans)
#
#print("Overall error: " + str(L12error))
#%%
fig = plt.figure(figsize=(5,5))
ax = fig.add_subplot(1,1,1)
ax.scatter(actual,testresult,c="b",s=10)
...
#%% Testing... debias the error calculation

# create an array with the predicted and actual values
testactual = np.concatenate((testresult,np.expand_dims(actual,1)),axis=1)

# sort the array by the actual values
testactualrank = testactual[np.argsort(testactual[:,1])]
...
idxremove = np.empty((0))
distmu = np.mean(testactual[:,0]) 
distsigma = np.std(testactual[:,0])/2
while len(idxremove) < testactual.shape[0]/4:
    newidx = find_nearest(testactualrank[:,0],np.random.normal(distmu,distsigma,1)) 
    if newidx not in idxremove:
        idxremove = np.append(idxremove,newidx)

testactualrankdeci = np.delete(testactualrank,idxremove,axis=0)

error = np.sqrt(mean_squared_error(testactualrank[:,0],testactualrank[:,1]))
errordeci = np.sqrt(mean_squared_error(testactualrankdeci[:,0],testactualrankdeci[:,1]))
print(error, "becomes", errordeci)

fig1 = plt.figure(figsize=(5,5))
ax = fig1.add_subplot(1,1,1)
ax.hist(testactualrankdeci[:,0], density=False, bins=30)
plt.grid(True)
plt.show()

fig = plt.figure(figsize=(5,5))
ax = fig.add_subplot(1,1,1)
ax.scatter(testactualrankdeci[:,1],testactualrankdeci[:,0],c="b",s=10)
ax.plot([0,50,120],[0,50,120])
plt.ylim(-5,110)
plt.xlim(-5,110)
plt.xlabel('Actual Results')
...
ax.hist(testresult, normed=True, bins=30)
